...
in_progress_compliments = [
    ...
    "О, гениальная! Работа {} обещает стать шедевром для других!",
    "Твой прогресс {} вдохновляет. Продолжай для клиентов!",
    "Мастер, твой этап {} — это путь к величию для других!",
    "О, творческая душа! Процесс {} дышит будущим искусством для клиентов!",
    "Твой процесс {} — как чернила судьбы в движении для других!",
    "Мастер! Работа {} — это начало легенды для клиентов!",
    "О, великая! Твой прогресс {} зовёт к шедеврам для других!"
]

equipment_compliments = [
    "Мастер! Твоё оборудование {} — основа великих тату для клиентов!",
    "О, гениальная! Инструмент {} в твоих руках творит чудеса для других!",
    ...
    "Твой инструмент {} — как рука богов для тату других!",
    "Мастер! Арсенал {} вдохновляет на великие дела для клиентов!",
    "О, великая! Твоё оборудование {} — это храм мастерства для других!"
]
...
def get_compliment(caption):
    ...
    if image_type == "sketch":
        ...
        compliment = random.choice(in_progress_compliments).format(caption)
    elif image_type == "equipment":
        compliment = random.choice(equipment_compliments).format(caption)
    else:
        compliment = random.choice(tattoo_compliments).format(caption)
